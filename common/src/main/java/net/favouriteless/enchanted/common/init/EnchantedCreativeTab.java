...
public class EnchantedCreativeTab {
    ...
    private static final List<String> PREGEN_KEYS = List.of(
            "enchanted:small_circle", "enchanted:medium_circle", "enchanted:large_circle"
    );
    private static final List<Supplier<Block>> PREGEN_BLOCKS = List.of(
    ...
    public static final Supplier<CreativeModeTab> TAB = register("main",
            ...
            (params, out) -> {
                ...
                for(Supplier<Block> blockSupplier : PREGEN_BLOCKS) {
                    for(String key : PREGEN_KEYS) {
                        ItemStack stack = new ItemStack(EItems.CIRCLE_TALISMAN.get());

                        CompoundTag tag = new CompoundTag();
                        tag.putString(key, BuiltInRegistries.BLOCK.getKey(blockSupplier.get()).toString());
                        stack.getOrCreateTag().put(CircleTalismanItem.SHAPE_TAG, tag);
